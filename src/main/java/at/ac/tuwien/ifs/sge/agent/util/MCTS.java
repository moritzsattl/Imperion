...
import at.ac.tuwien.ifs.sge.agent.util.MacroAction.AbstractMacroAction;
...
public class MCTS {
    ...
    public Tree<ImperionGameNode> expansion(Tree<ImperionGameNode> bestNode) {
        ...
        if (bestNode.isRoot()) {
            ...
            Imperion.assertWithMessage(gameState.hasUnexploredActions(), "Why are there no unexplored actions?");
     ...
     * Each simulation just simulates what would happen if the scheduled commands in the command queue would be executed
     */
    public boolean[] simulation(Tree<ImperionGameNode> tree, long nextDecisionTime) {
        ...
        try {
            while (!game.isGameOver() && depth++ <= DEFAULT_SIMULATION_DEPTH && System.currentTimeMillis() < nextDecisionTime
                ...
                //possibleActions.add(new DoNothingMacroAction());

                //Imperion.logger.debug("Possible actions: " + possibleActions);

                // Add random macroAction to commandQueue
                //var macroAction = Util.selectRandom(possibleActions);
                //commandQueues[playerToTurn].addCommand(macroAction);
                //Imperion.logger.debug("Added action:" +macroAction);
    ...
    /**
     * Try to schedule the next action in queue for each unit ar city and advances game
     */
     ...
     */
    private static boolean schedule(Empire game, int playerToTurn, Map.Entry<?, Queue<EmpireEvent>> command, ArrayList<EmpireEvent> scheduledEvents) {
    ...
    private boolean[] determineWinner(Empire game) {
        ...
        if (game.isGameOver()) {
            ...
            winners[agent.getPlayerId()] = Heuristics.isWinner(game, agent.getPlayerId());
        }
        return winners;
    }
